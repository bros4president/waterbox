...
### 2026-09-03 - Live acceptance workflow readiness

- Implemented: updated the separately authorized Box and Vercel MCP smoke workflows to require an explicit automatic-stop duration; expect the complete MCP catalog including `stop_sandbox`; prove explicit stop followed by exactly one ordinary tool invocation and implicit resume; observe native automatic stop through bounded read-only probes and prove resumability with exactly one later ordinary tool invocation; and write, restore, and read a marker through relative paths and the provider-default command workspace. After snapshot work, one no-cwd ordinary command first establishes fresh running compute before the automatic-stop clock begins, including when Vercel manual snapshot creation stopped the source. Valid public resource identifiers from successful or malformed results, plus exact validated recovery sandbox IDs from MCP creation errors, are captured before complete response validation so every identifiable created sandbox and snapshot enters single-attempt cleanup without a creation replay. Box preflight now reserves capacity for the two concurrent Boxes required by snapshot restore, while the existing Box and Vercel exact bounded baseline reconciliation remains mandatory.
- Verification: `bun test scripts/direct-mcp-smoke.test.ts scripts/vercel-mcp-smoke.test.ts` passed (30 tests), including both provider workspace layouts, snapshot-induced source stopping before automatic-stop arming, lifecycle stage ordering, pre-network automatic-stop gating, bounded automatic-stop timeout, recovery-ID and pre-validation resource tracking, cleanup continuation, exact baseline comparison, redaction, and no mutation replay; `bun run typecheck` passed; `git diff --check` passed.
- Live evidence: not run. Credential presence was checked without displaying values, and the provider-specific destructive-operation and isolation gates remain required. No provider request was made during workflow preparation.
- Deviations: exact provider resume-dispatch counts remain established by the credential-free adapter/core regressions; the live MCP flow establishes one logical ordinary-tool invocation after each observed stopped state without adding provider mutation tracing.

### 2026-09-03 - Live automatic-stop target correction

- Corrected: automatic-stop observation now probes, arms, waits on, and ordinarily resumes the restored sandbox whose relative marker and current runtime were just verified. The source sandbox is deliberately made runtime-stale before the snapshot and is never used for post-snapshot arming or automatic-stop evidence. This separates automatic-stop acceptance from Vercel's snapshot-induced source stopping and from the smoke's intentional source runtime invalidation.
- Verification: `bun test scripts/direct-mcp-smoke.test.ts scripts/vercel-mcp-smoke.test.ts` passed (30 tests, 114 assertions), including a post-snapshot stale-source vector that permits no source probe or Bash operation while proving restored-target arming and resume; `bun run typecheck` passed; `git diff --check` passed.
- Live evidence: no provider call ran for this correction. A prior separately authorized Vercel run exposed the stale-source arming defect and completed its tracked cleanup; the corrected live acceptance run remains pending.
- Deviations: none.

### 2026-09-03 - Live-discovered Box stop completion correction

- Corrected: a validated `box.stopping` response now marks accepted stop dispatch rather than completed stop. The Box adapter performs bounded exact read-only polling until `archived` and only then returns `stopped`. Definite pre-acceptance 4xx and rate-limit responses remain failures; after acceptance, polling transport failures, timeouts, non-authoritative failures, 4xx responses, and rate limits become `ambiguous_execution`. Exact absence remains `exact_absence` with a terminated observation, native error remains `known_state` with a failed observation, and no mutation is retried. Primitive and assembled-service regressions prove accepted-to-archived completion, exact `stopped` returned through core before one subsequent delete, and retained `stopping` with one stop dispatch after polling ambiguity.
- Verification: `bun test packages/sandbox-provider-box/test/provider.test.ts packages/sandbox-provider-runtime/test packages/sandbox-core/test/service.test.ts packages/mcp/test/server.test.ts` passed (139 tests); `bun test scripts/direct-mcp-smoke.test.ts` passed (24 tests), including exact stopped-state validation before the ordinary-resume and tracked-cleanup stages; `bun run typecheck` passed; `bun run test:node-sqlite` passed (1 test); `git diff --check` passed.
- Live evidence: no provider call ran for this correction. A prior separately authorized Box run exposed the premature stop-completion defect; corrected live acceptance remains pending.
- Deviations: none.

### 2026-09-03 - Live-discovered Box resume command-readiness correction

- Corrected: a Box `ready` or `idle` observation after an accepted resume is now followed by a bounded provider-level readiness probe through the installed Waterbox launcher before the adapter returns `running`. A deterministic not-ready probe result is polled without replaying the accepted resume; an uncertain probe result remains `ambiguous_execution` and is not replayed. This keeps one ordinary tool call responsible for one implicit resume while ensuring the user's command is dispatched exactly once only after the provider proves that the restored workspace and runtime can execute it.
- Verification: credential-free primitive regressions reproduce a resumed-but-not-command-ready response followed by exact readiness, prove one resume dispatch, bound deterministic not-ready polling, and prove that a lost readiness response triggers neither lifecycle nor probe replay. An assembled core/provider regression proves two harmless readiness probes followed by exactly one user-command dispatch and a durable `running` result. `bun test packages/sandbox-provider-box/test/provider.test.ts packages/sandbox-provider-runtime/test packages/sandbox-core/test/service.test.ts packages/mcp/test/server.test.ts scripts/direct-mcp-smoke.test.ts` passed (167 tests); `bun run typecheck` passed; `bun run test:node-sqlite` passed (1 test); `git diff --check` passed.
- Live evidence: a separately authorized Box run reached exact explicit `stopped`, then the first ordinary post-stop tool call received a deterministic nonzero provider command result with no meaningful stderr and Waterbox conservatively surfaced `ambiguous_execution`. Its exact tracked sandbox cleanup completed and both pre-run visible-ID and active-count baselines converged. No credential or provider-native identifier was recorded. Corrected Box live acceptance remains pending.
- Deviations: none.

### 2026-09-03 - Live-discovered Vercel automatic-stop inspection correction

- Corrected: Vercel-native automatic-shutdown `stopping` and `snapshotting` responses now remain below the provider boundary. Exact owned-name inspection polls those transient responses read-only, with resume explicitly disabled, until Vercel supplies an authoritative `running`, `stopped`, or `failed` response or exact absence; the existing provider polling deadline bounds the sequence. This prevents a provider-native shutdown phase from being mistaken for Waterbox's durable `stopping` workflow checkpoint, which remains reserved for a Waterbox-owned stop mutation.
- Verification: credential-free primitive coverage proves native `stopping` through `snapshotting` to the documented stopped response shape, exact failure and absence, bounded timeout, and exclusively non-resuming GET inspection. An assembled provider/core regression starts from cached `running`, persists only the exact observed `stopped` state, and then proves one ordinary tool invocation causes exactly one implicit resume and one user-command dispatch. `bun test packages/sandbox-provider-vercel/test/infrastructure.test.ts packages/sandbox-provider-runtime/test packages/sandbox-core/test/service.test.ts` passed (124 tests); `bun run typecheck` passed; `bun run test:node-sqlite` passed (1 test); `git diff --check` passed.
- Live evidence: no provider call ran for this correction, and no credential source was accessed. The prior separately authorized Vercel run exposed the inspection-boundary defect and completed exact tracked cleanup; corrected Vercel live acceptance remains pending.
- Deviations: none.
