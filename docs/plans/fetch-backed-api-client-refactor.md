...
- 2026-09-01: Phase 6 second release-review follow-up made the remaining parity assertions explicit in both compositions. Authentication now proves the embedded private wrapper replaces caller authorization while the underlying raw handler and real listener reject missing/wrong credentials and preserve asynchronous resolver aborts. Artifact-aware fake preparation is observed without Box or a live credential. Definite and ambiguous post-checkpoint failures retain canonical redacted recovery IDs; cancellation winning after the checkpoint exposes no invented ID and requires caller-controlled same-key replay. Provider-ready persisted `preparing` remains sticky under probe, while referenced `provisioning` is inspected and prepared through success and canonical recovery failure. Ordinary get is proven not to probe a running sandbox. Dispatched Bash now emits an actual nonterminal observation, produces strictly positive progress, cannot clean before terminal drain, never cleans a fallback receipt, truncates after multiple offset samples, and treats cleanup failure as bounded best effort. Dependency guards inspect dependencies, optional dependencies, peer dependencies, and development dependencies with self-tests for every map and a narrow MCP artifact-loader allowance. Final verification passed 424 repository tests, typecheck, MCP and sandbox CLI builds, 9 API-local integration tests, all three Node syntax checks, MCP package dry-run with exactly six intended files, and `git diff --check`. No secret was read, no live provider request occurred, and no runtime state leaked.
- 2026-09-01: Phase 6 artifact review follow-up made artifact-backed parity causal rather than decorative. The injected fake provider now requires a concrete caller-owned runtime artifact at construction, validates its bytes against the injected SHA-256 and exact protocol/version facts, revalidates it during preparation, derives a preparation token from those facts, and records the exact consumed artifact identity. Both embedded and network cases assert that identity and token. Omitted and substituted artifacts fail construction, while mutation after construction fails preparation with a canonical recovery handle and records no consumption. Final verification passed 426 repository tests and 31 focused parity/guard tests, typecheck, MCP and sandbox CLI builds, 9 API-local integration tests, all three Node syntax checks, MCP package dry-run with exactly six intended files, and `git diff --check`. No secret was read, no live provider request occurred, and no runtime state leaked.
