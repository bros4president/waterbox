...
Status: in progress; Phases 0-2 complete; Phases 3-6 pending
...
- 2026-09-01: Phase 2 completed. Added the private `@waterbox/client` workspace package with the deployment-neutral `ApiBackend`, validated no-retry remote backend, idempotent ownership cleanup, product-level commands for the complete supported MCP surface, bounded JSON/error/NDJSON decoding, safe typed API and transport errors, validated recovery sandbox IDs, caller-controlled creation replay, composite Bash observation/progress/truncation/receipt-fallback/detached-cleanup policy, and caller-byte secure transfer using client-side age encryption. The package depends only on contracts and client-side libraries and contains no host-path or presentation logic. Focused client and API verification passed 43 tests, repository typecheck passed, and `git diff --check` passed. Tests used injected Fetch fakes and ephemeral encryption keys; no credential value was read and no live provider request occurred.
