...
describe("Direct MCP smoke", () => {
  ...
  test("runs one fresh current-runtime core flow and deletes its returned public ID", async () => {
    const fake = fakeClient()
    await runDirectMcpProductFlow(fake.client, { localSecretPath: "/local/secret", sleep: fake.sleep })
    ...
    expect(fake.calls.some((call) => call.name === "create_snapshot" || call.name === "list_snapshots")).toBe(false)
  ...
  test("does not clean up before a public ID and redacts create failure output", async () => {
...
const resource = (state = "running") => ({ sandboxId, provider: "box", state, version: 1, createdAt: now, updatedAt: now })

function fakeClient(options: { failRuntime?: boolean; hardTimeout?: boolean } = {}) {
  ...
  const client = {
    ...
    async callTool(request: Call): Promise<any> {
      calls.push(request)
      if (request.name === "create_sandbox" || request.name === "probe_sandbox") return text(resource())
      if (request.name === "delete_sandbox") { deletions.push(request.arguments.sandboxId); return text(resource("terminated")) }
      if (request.name === "send_file_securely") return text({ bytes: 5 })
      if (request.name === "read") return output("Alpha\n")
      ...
      if (command.includes("waterbox health")) { if (options.failRuntime) throw new Error("runtime unavailable"); return bash(`${JSON.stringify({ ok: true, protocolVersion: 2, tools: ["read", "write", "edit", "patch", "glob", "grep", "bash"] })}\n${JSON.stringify({ protocolVersion: 2 })}\n`) }
