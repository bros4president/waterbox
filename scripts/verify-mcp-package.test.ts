...
describe("MCP installed-artifact verifier", () => {
  test("selects only npm's single reported tarball and rejects decoys", async () => {
    ...
    await writeFile(join(directory, "waterbox-mcp-0.1.0-alpha.1.tgz"), "exact")
    expect(await resolveOnlyPackedTarball(directory, "waterbox-mcp-0.1.0-alpha.1.tgz")).toBe(join(directory, "waterbox-mcp-0.1.0-alpha.1.tgz"))
    await writeFile(join(directory, "decoy.tgz"), "decoy")
    await expect(resolveOnlyPackedTarball(directory, "waterbox-mcp-0.1.0-alpha.1.tgz")).rejects.toThrow("exactly one")
    await expect(resolveOnlyPackedTarball(directory, "../waterbox-mcp-0.1.0-alpha.1.tgz")).rejects.toThrow("invalid tarball")
  })
  ...
  test("enforces exact CLI-only package surface and Node-only bundles", () => {
    ...
    const manifest = { name: "waterbox", version: "0.1.0-alpha.1", engines: { node: ">=24.15.0" }, license: "MIT", bin: { waterbox: "./dist/waterbox.js" }, publishConfig: { access: "public", tag: "next" } }
    expect(() => assertCliOnlyManifest(manifest)).not.toThrow()
    expect(() => assertCliOnlyManifest({ ...manifest, publishConfig: { access: "public", tag: "latest" } })).toThrow("channel")
    expect(() => assertCliOnlyManifest({ ...manifest, publishConfig: { tag: "next" } })).toThrow("channel")
