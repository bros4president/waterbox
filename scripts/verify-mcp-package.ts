...
export function assertCliOnlyManifest(manifest: Record<string, any>): void {
  if (manifest.name !== "waterbox" || manifest.version !== "0.1.0" || manifest.engines?.node !== ">=24.15.0" || manifest.license !== "MIT") throw new Error("Packed manifest identity, version, engine, or license is invalid")
...
export async function verifyMcpPackage(): Promise<void> {
  ...
  try {
    ...
    if (process.env.WATERBOX_RELEASE_ARTIFACT_DIR) {
      ...
      await copyFile(first.identity.path, join(output, "waterbox-0.1.0.tgz"))
      assertSameBytes(await readFile(first.identity.path), await readFile(join(output, "waterbox-0.1.0.tgz")), "Retained release tarball")
      await writeFile(join(output, "waterbox-0.1.0.sha256"), `${first.identity.sha256}  waterbox-0.1.0.tgz\n`)
      await writeFile(join(output, "waterbox-0.1.0.files.sha256"), `${(await contentManifest(first.extractDirectory)).join("\n")}\n`)
    }
...
async function verifyImplementationVersions(): Promise<void> {
  const [server, composition] = await Promise.all([readFile(join(packageRoot, "src/server.ts"), "utf8"), readFile(join(root, "packages/control-plane-local/src/index.ts"), "utf8")])
  if (!server.includes('new Server({ name: "waterbox", version: "0.1.0" }') || !composition.includes('loadSandboxRuntimeArtifact(artifactLocation, "0.1.0")')) throw new Error("Package, MCP server, and sandbox artifact versions are not aligned at 0.1.0")
}
...
async function buildAndPack(temporaryRoot: string, name: string) {
  ...
  await run("node", ["scripts/build-waterbox.mjs", "mcp"], { cwd: sourceRoot, maxBuffer: 8 * 1024 * 1024 })
...
async function protocolSmoke(node: string, executable: string, temporaryRoot: string, environment: NodeJS.ProcessEnv): Promise<void> {
  ...
  if (called.result?.isError !== true || !called.result?.content?.[0]?.text?.toLowerCase().includes("run waterbox setup")) throw new Error("Packaged MCP did not return unconfigured setup guidance")
  child.stdin.end()
...
function cleanEnvironment(home: string, path = process.env.PATH): NodeJS.ProcessEnv {
  ...
  env.PATH = path
  for (const key of ["WATERBOX_PROVIDER", "WATERBOX_AUTO_STOP", "WATERBOX_MCP_DIAGNOSTICS", "BOX_API_KEY", "BOX_API_BASE_URL", "BOX_POLL_INTERVAL_MS", "BOX_POLL_TIMEOUT_MS", "VERCEL_TOKEN", "VERCEL_API_ORIGIN", "VERCEL_TEAM_ID", "VERCEL_PROJECT_ID", "VERCEL_POLL_INTERVAL_MS", "VERCEL_POLL_TIMEOUT_MS", "VERCEL_REQUEST_TIMEOUT_MS"]) delete env[key]
  return env
}
