...
export async function runCli(arguments_: string[], dependencies: CliDependencies = {}): Promise<number> {
  ...
  try {
    ...
    if (arguments_[0] === "logout") { await logout(storage, credentials); write("Waterbox configuration removed."); return 0 }
    ...
    if (environment.WATERBOX_PROVIDER === undefined && !providerVariablesPresent(environment)) {
      ...
      if (!config) { write("Waterbox status: not configured. Run waterbox setup."); return 0 }
      ...
      write(`Waterbox status: provider ${config.provider}; configuration keyring; credential ${state}.${state === "available" ? "" : ` Environment fallback: WATERBOX_PROVIDER=${config.provider} and ${credentialVariable(config.provider)}.`}`)
      if (state !== "available") return 1
      return 0
    }
    const parsed = await parseMcpConfig(environment, undefined, { storage, credentials })
    if (parsed.provider.type !== "local") throw new Error("Waterbox Cloud is not supported. Run waterbox setup for Box or Vercel.")
