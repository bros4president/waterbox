...
import { z } from "zod"
import { automaticStopEnvironmentValue, deriveProviderConfigurationId, normalizeBoxApiBaseUrl, normalizeVercelApiOrigin, parseAutomaticStopDuration, type LocalProviderBindingInput } from "@waterbox/control-plane-local"
...
export async function setup(storage: ConfigStorage, credentials: CredentialStore, prompts: SetupPrompts): Promise<Provider> {
  ...
  const prospectiveBinding = bindingFor(config, secret)
  const currentBinding = priorConfig === undefined || prior[priorConfig.provider] === undefined ? undefined : bindingFor(priorConfig, prior[priorConfig.provider]!)
  if (currentBinding !== undefined && prospectiveBinding !== currentBinding) {
  ...
  let selectedMutated = false, configCommitted = false
  try {
