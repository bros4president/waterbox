...
export class BoundaryError extends Error {
  readonly _tag = "Patch.BoundaryError"

  constructor(readonly boundary: "first" | "last") {
    super(
      `The ${boundary} line of the patch must be '${boundary === "first" ? "*** Begin Patch" : "*** End Patch"}'`,
    )
...
export class InvalidHunkError extends Error {
  readonly _tag = "Patch.InvalidHunkError"
  readonly line: string
  readonly lineNumber: number
  readonly reason?: string

  constructor(options: { line: string; lineNumber: number; reason?: string }) {
    const message = options.reason
      ? `Invalid hunk at line ${options.lineNumber}: ${options.reason}`
      : `Invalid hunk at line ${options.lineNumber}: '${options.line}' is not a valid hunk header. Valid hunk headers: '*** Add File: {path}', '*** Delete File: {path}', '*** Update File: {path}'`
    super(message)
...
export function parse(patchText: string): ReadonlyArray<Hunk> {
  ...
  if (begin === -1) throw new BoundaryError("first")
  if (end === -1 || begin >= end) throw new BoundaryError("last")
  ...
  while (index < end) {
    ...
    if (header.startsWith("*** Delete File: ")) {
      ...
      if (index + 1 < end && next !== undefined && !isBoundary(next)) {
        if (next.startsWith("*** ")) throw new InvalidHunkError({ line: next, lineNumber: index + 2 })
    ...
    throw new InvalidHunkError({ line: header, lineNumber: index + 1 })
  }
  return hunks
}
...
function parseUpdate(
  ...
  path: string,
  hunk: number,
): { chunks: ReadonlyArray<UpdateFileChunk>; next: number } {
  const chunks: Array<{
    oldLines: string[]
    newLines: string[]
...
): ReadonlyArray<readonly [start: number, remove: number, insert: ReadonlyArray<string>]> {
...
function seek(lines: ReadonlyArray<string>, pattern: ReadonlyArray<string>, start: number, eof = false): number {
