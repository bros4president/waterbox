...
export class BoxSandboxProvider implements SandboxProvider {
  ...
  async *executeTool<N extends ToolName>(input: ProviderExecuteInput<N>): AsyncIterable<ToolEventByName[N]> {
    ...
    const ref = sandboxRef(input.providerRef)
    const bashTimeout = input.toolName === "bash" ? (input.arguments as { timeout?: number }).timeout : undefined
    if (typeof bashTimeout === "number" && bashTimeout > 580_000) throw new ProviderError("failure", "The requested bash timeout exceeds the Box command limit")
    let payload: string
...
function cliError(stdout: string): { status: number; code: string } | undefined { try { const value = JSON.parse(stdout.trim()); return isExactObject(value, ["protocolVersion", "type", "status", "code"]) && value.protocolVersion === 1 && value.type === "error" && Number.isInteger(value.status) && typeof value.code === "string" ? { status: Number(value.status), code: value.code } : undefined } catch { return undefined } }
